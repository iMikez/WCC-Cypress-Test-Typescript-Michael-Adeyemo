describe('Report a Noise Complaint Form Test', () => {
  //Test Scenario 1
  ...
  it('Verify that user can submit noise report form (No option)', () => {
    ...
    cy.url().should('include', '/your-details');
    cy.contains('No').click(); //If the user selects 'No' there should be no additional fields except terms & conditions checkbox 
    cy.get('.square-checkbox[name="termsAndConditionsAccepted"]').check();
